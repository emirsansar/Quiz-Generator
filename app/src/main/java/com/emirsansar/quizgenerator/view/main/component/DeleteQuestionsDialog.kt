...
@Composable
fun DeleteQuestionsDialog(
    onDeleteConfirmed: () -> Unit,
    onDismiss: () -> Unit
) {
    AlertDialog(
        onDismissRequest = { onDismiss() },
        title = { Text(stringResource(id = R.string.header_questions_for_delete_dialog), fontSize = 18.sp, color = MaterialTheme.colorScheme.onBackground) },
        text = {
            Text(stringResource(id = R.string.text_dialog_all_questions), color = MaterialTheme.colorScheme.onBackground)
        },
        confirmButton = {
            TextButton(onClick = {
                onDeleteConfirmed()
